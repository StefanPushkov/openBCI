...
def auto_markup(filename: str, markup_margin=0):
    ...
    def set_left_hand():
        ...
        for i in left_interval:
            set_class(start=i[0]-markup_margin, stop=i[0], classname=1)
    ...
    def set_right_hand():
        ...
        for i in left_interval:
            set_class(start=i[0]-markup_margin, stop=i[0], classname=2)

    set_left_hand()
    set_right_hand()
    data = data.drop(['N', 'right', 'left', 'time'], axis=1)
    out = cf.base_dir+'/converted_data/markup_class{0}.csv'.format(num)
    data.to_csv(out, index=False, header=False)
    print(out)
    return out # "E:/exp/converted_data/markup_class{0}.csv".format(num)
